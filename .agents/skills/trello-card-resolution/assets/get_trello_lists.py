...
from api import TrelloClient, env_get, load_env_file  # noqa: E402


def parse_args():
    parser = argparse.ArgumentParser(description="Fetch Trello lists and write to file or stdout.")
...
def main():
    ...
    output_path = trello_dir / "trello_lists.json"
    ...
    board_id = os.getenv("TRELLO_BOARD_ID") or env_get("TRELLO_BOARD_ID", env_values, env_path)
    client.get(f"boards/{board_id}", fields="id")
    ...
    payload = {
        ...
        "lists": sorted(lists, key=lambda item: item.get("name", "").lower()),
    }
    ...
    print(f"Done! Updated {output_path} with {len(lists)} lists.")
